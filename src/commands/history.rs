...
pub async fn execute(key: Option<String>, limit: Option<usize>, vault: Option<String>) -> Result<()> {
    ...
    if !Git::is_repo(vault_path)? {
        println!("{}", "No version history available. Initialize git with 'bunker init' to track changes.".yellow());
        return Ok(());
    }
    ...
    for (i, commit) in history.iter().enumerate() {
        ...
        println!("{} {} {}", prefix, commit.hash[..8].yellow(), commit.message);
        println!("{}   {} by {} on {}", 
