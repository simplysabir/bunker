...
pub async fn execute(password: String, output: Option<PathBuf>, vault: Option<String>) -> Result<()> {
    ...
    println!("📦 Export file: {}", output_path.display().to_string().cyan());
    ...
    println!("  2. Run: {}", format!("bunker vault import {} <password> <vault-name>", 
        output_path.file_name().unwrap().to_string_lossy()).white().bold());
    ...
    if utils::prompt_confirm("Generate import command for easy copy-paste?")? {
        let import_cmd = format!("bunker vault import {} {} {}", 
