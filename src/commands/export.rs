...
pub async fn execute(
            ...
            if let Ok(decrypted) = Crypto::decrypt(&entry.value, &master_key) {
                if let Ok(value) = String::from_utf8(decrypted) {
                    let export_entry = ExportEntry {
                        ...
                        notes: if include_metadata { entry.metadata.notes } else { None },
                        tags: if include_metadata { entry.metadata.tags } else { Vec::new() },
    ...
    let content = match format.as_str() {
        ...
        _ => return Err(anyhow!("Unsupported format: {}. Use json, csv, or pass", format))
    };
    
    // Write to file or stdout
    if let Some(path) = output {
        fs::write(&path, content)?;
        println!("{} Exported {} entries to {}", 
            "✓".green().bold(), 
            entry_count, 
