...
pub async fn execute(
            ...
            for line in lines {
                ...
                if parts.len() >= 2 {
                    let entry = ExportEntry {
                        ...
                        username: if parts.len() > 2 && !parts[2].is_empty() { Some(parts[2].to_string()) } else { None },
                        url: if parts.len() > 3 && !parts[3].is_empty() { Some(parts[3].to_string()) } else { None },
                        notes: if parts.len() > 4 && !parts[4].is_empty() { Some(parts[4].to_string()) } else { None },
    ...
    println!("{} Imported {} entries", "✓".green().bold(), imported.to_string().cyan());
    if skipped > 0 {
        println!("{} Skipped {} existing entries (use --overwrite to replace)", 
            "⚠".yellow(), 
