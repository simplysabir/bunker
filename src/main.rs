...
async fn main() -> Result<()> {
    ...
    // Execute command
    match command {
        Commands::Init { name, non_interactive } => {
            commands::init::execute(name, non_interactive, cli.vault).await
        }
        
        Commands::Add { key, value, note, file } => {
            commands::add::execute(key, value, note, file, cli.vault).await
        }
        
        Commands::Get { key, copy, timeout } => {
            commands::get::execute(key, copy, cli.vault).await
        }
        
        Commands::Edit { key, value } => {
            commands::edit::execute(key, value, cli.vault).await
        }
        
        Commands::Remove { key, force } => {
            commands::remove::execute(key, force, cli.vault).await
        }
        
        Commands::List { tree } => {
            commands::list::execute(None, tree, cli.vault).await
        }
        
        Commands::Search { query } => {
            commands::search::execute(query, cli.vault).await
        }
        
        Commands::Generate { 
            length, uppercase, lowercase, numbers, symbols, 
            no_ambiguous, charset, copy 
        } => {
            ...
            commands::generate::execute(None, length, !symbols, !numbers, !uppercase, cli.vault).await
        }
        ...
        Commands::Peek { key, chars } => {
            commands::peek::execute(key, cli.vault).await
        }
        
        Commands::Move { from, to } => {
            commands::move_cmd::execute(from, to, cli.vault).await
        }
        ...
        Commands::Vault { action } => {
            match action {
                ...
                VaultAction::List => {
                    commands::vault::execute(cli::VaultAction::List).await
                }
        ...
        Commands::Lock => {
            commands::lock::execute(cli.vault).await
        }
        
        Commands::Unlock { duration } => {
            commands::unlock::execute(cli.vault, Some(duration)).await
        }
        
        Commands::Status => {
            commands::status::execute(cli.vault).await
        }
        
        Commands::Backup { destination } => {
            commands::backup::execute(destination, cli.vault).await
        }
        ...
        Commands::Env { key, var } => {
            commands::env::execute(key, var, cli.vault).await
        }
    }
}
